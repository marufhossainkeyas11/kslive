...
function buildQualityList() {
  ...
  if (state.dash) {
    ...
    // ভিডিও height দিয়ে ডিডুপ (একই height একাধিক audio variant-এ রিপিট হয়)
      ...
      .forEach(t => {
        ...
        row.addEventListener('click', () => {
          try {
            ...
            // active flag আপডেট হতে একটু সময় লাগে, তাই কাঙ্ক্ষিত height জোর করে badge-এ বসাও
...
function collectAudioTracks() {
  ...
  if (state.dash) {
    ...
    // audioId দিয়ে ডিডুপ করা — একই audio track একাধিক video variant-এ রিপিট হয়
...
function setActiveAudio(id) {
  ...
  if (track.kind === 'hls' && state.hls) {
    ...
    try {
      ...
      // একই audioId-র variant গুলোর মধ্যে বর্তমান video quality-র সাথে মিলে এমনটা বেছে নাও
      const currentVideo = variantTracks.find(t => t.active);
      let target = variantTracks.find(t =>
        t.audioId === track.ref && currentVideo && t.height === currentVideo.height
      );
...
function collectCcTracks() {
  ...
  // ── Native <track> elements (HLS Safari native পাথ) ──
  ...
  // trackschanged ইভেন্ট বারবার ফায়ার হতে পারে (এমনকি আমরা নিজেরাই track
  // সিলেক্ট করলেও) — তাই আগে থেকে কিছু সিলেক্ট করা থাকলে সেটা পুনরায় খুঁজে
  // বসাও, শুধুমাত্র প্রথমবার (কিছুই সিলেক্ট করা ছিল না) হলে OFF রাখো।
  if (prevActiveId !== null && tracks.some(t => t.id === prevActiveId)) {
    // ইতিমধ্যে যেটা সিলেক্ট করা ছিল সেটাই আবার রি-অ্যাপ্লাই করছি, কিন্তু
    // collectCcTracks-কে অসীম লুপে ফেলা এড়াতে state সরাসরি বসাচ্ছি,
    // setActiveCc() আবার কল করছি না।
    state.activeCcId = prevActiveId;
    refreshCcButtons();
    buildCcList();
  } else {
    setActiveCc(null); // প্রথমবার — default: off
  }
}
...
async function setActiveCc(id) {
  ...
  if (track.kind === 'hls' && hls) {
    ...
    try {
      // visibility(true) আগে পুরোপুরি শেষ হতে দাও — এর ভেতরের internal
      // switch logic-ই active track ঠিক করে, তাই এটা resolve না হলে
      // selectTextTrack() নিঃশব্দে ignore হয়ে যায়।
      ...
      const freshMatch = liveTracks.find(lt =>
        lt.language === track.ref.language && lt.label === track.ref.label
      ) || track.ref;
